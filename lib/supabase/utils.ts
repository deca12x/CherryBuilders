...
/**
 * A utility function to get the profile picture's public url
 * @param address - The address of the user
 * @param fileName - The name of the file containing the profile picture of the user
 * @returns The public url
 **/
export const getProfilePicturePublicUrl = async (address: string, fileName: string): Promise<string> => {
  return await (await fetch(`/api/bucket/profile-pictures/${address}?fileName=${fileName}`)).json();
};

// Will probably be removed -------------------------------------------

/**
 * A utility function to get supabase realtime channels
 * @returns The realtime channels of supabase
 **/
export const getSupabaseRealtimeChannels = async (): Promise<RealtimeChannel[]> => {
  return await (await fetch("/api/supabase/channels")).json();
};

/**
 * A utility function to remove supabase realtime channels
 * @param - The ref to the channel that must be removed
 **/
export const removeSupabaseRealtimeChannel = async (
  channel: MutableRefObject<any>
): Promise<"ok" | "timed out" | "error"> => {
  return await (
    await fetch("/api/supabase/channels", {
      method: "DELETE",
      headers: {
        Authorization: `Bearer jwt`, // TODO: add the correct jwt
        "Content-Type": "application/json",
